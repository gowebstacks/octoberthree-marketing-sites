...
const generateTocItems = (body: any) => {
  ...
  return blocks
    ...
    .map((block: any, index: number) => ({
      key: `toc-${index}`,
      title: block.content.map((n: any) => n.text || "").join("").trim(),
    }));
};

const extractAuthorAndForm = (body: any) => {
  if (!body?.content) return { hasPair: false, authorCard: null, formBlock: null, filteredContent: body };
...
export const PortableText: FC<{ blok: PortableTextProps }> = ({ blok }) => {
  ...
  const { hasPair, authorCard, formBlock, filteredContent } = extractAuthorAndForm(blok.body);
...
//       const reachedTop = top <= 0;

//       console.log("📍 Element top:", top);
//       console.log("📌 Has reached top:", reachedTop);
...
//     ref={ref}
//     className={
//       twMerge(hasToc && isAtTop && "overflow-auto h-full")
      
//     }>
//       {(hasPair &&  authorCard && formBlock ) ? (
//         <>
//         {renderSideBySide(authorCard, formBlock)}
//         {renderDefault(blok, filteredContent, hasToc)}
//       </>
//       ) :
//       renderDefault(blok, blok.body, hasToc)
//     }
//     </div>
//   );
