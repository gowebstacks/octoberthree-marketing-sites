...
const getVideoUrl = (blok: VideoBlok, autoplay: boolean): string | null => {
  if (blok.videoType === "youtube" && blok.youtubeUrl) {
    ...
    const id = match?.[1];
    if (!id) return null;
    return `https://www.youtube.com/embed/${id}?autoplay=${autoplay ? 1 : 0}&mute=1&playlist=${id}`;
  }
  ...
  if (blok.videoType === "wistia" && blok.wistiaUrl) {
    ...
    if (!id) return null;
    return `https://fast.wistia.net/embed/iframe/${id}?autoPlay=${autoplay ? "true" : "false"}&muted=true`;
  }
  ...
  if (blok.videoType === "vimeo" && blok.vimeoUrl) {
    ...
    const id = match?.[1];
    if (!id) return null;
    return `https://player.vimeo.com/video/${id}?autoplay=${autoplay ? 1 : 0}&muted=1&playsinline=1`;
  }
...
export const Video: FC<{ blok: VideoBlok }> = ({ blok }) => {
  ...
  const effectiveAutoplay = isPlaying ? true : autoPlay;
  const videoUrl = getVideoUrl(blok, effectiveAutoplay);
      ...
      {isPlaying &&
        (isEmbed ? (
          ...
          <video
            ...
            playsInline
            muted={effectiveAutoplay}   // Mute is required for autoplay in most browsers
          >
            <track kind="captions" />
